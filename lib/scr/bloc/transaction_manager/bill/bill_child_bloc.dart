...
class BillManageChildBloc implements TransuctionManageChildBloc {
  @override
  Stream<TransuctionManageState> loadList(
    ...
    try {
      var response = await bloc.transManangerRepository.getTransactionList(
        ...
        TransactionManage.billCat,
      );

      if (response.result!.isSuccess()) {
        final ListResponse<BillPaymentModel> listTransaction = ListResponse<BillPaymentModel>(
          response.data,
              (item) => BillPaymentModel.fromJson(item),
        );
        List<BillPaymentModel> result = listTransaction.items;

        yield bloc.state.copyWith(
          listState: TransuctionManageListState<BillPaymentModel>(
        ...
        yield bloc.state.copyWith(
          listState: TransuctionManageListState<BillPaymentModel>(
            ...
            errorMessage: response.result?.getMessage(defaultValue: AppTranslate.i18n.errorNoReasonStr.localized),
          ),
        );
      }
    } catch (e) {
  ...
  @override
  Stream<TransuctionManageState> loadServiceList(TransuctionManageBloc bloc) async* {
    var response =
    await bloc.transManangerRepository.getTransactionServiceType(transCatKey: TransactionManage.fxCat.key);
