...
class TransactionSavingList extends StatelessWidget {
  ...
  Widget getChild() {
    List<TransactionGrouped<TransactionSavingModel>>? listData = transactionBloc.state
        .listState?.transactions
        ?.tryCast<TransactionGrouped<TransactionSavingModel>>();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, grpIdx) {
        TransactionGrouped<TransactionSavingModel> tg = listData[grpIdx];
        return StickyHeader(
          header: ShaderMask(
                ...
                .mapIndexed(
                  (t, transIdx) => Padding(
                    ...
                    child: TransactionListItem(
                      ...
                      transIndex: transIdx,
                      onPress: () {
                        _onItemPress(context, t, transactionBloc.state);
                      },
                    ),
                  ),
                )
  ...
  _onItemPress(BuildContext context, TransactionSavingModel model,
      TransuctionManageState state) async {
    pushNamed(
      context,
      SavingTransactionApproveScreen.routeName,
      arguments: SavingTransactionApproveScreenArguments(
        actionType: null,
        transactionCode: model.transCode ?? '',
      ),
      async: true,
    );
  }
