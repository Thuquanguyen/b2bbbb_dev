...
class _EnsureVisibleWhenFocusedState extends State<EnsureVisibleWhenFocused>
  ...
  Future<void> _ensureVisible() async {
    ...
    Logger.debug('_ensureVisible hasFocus');
    ...
    Logger.debug('_ensureVisible viewport');
    ...
    Logger.debug(
        'check viewport ${position.pixels} ${viewport.getOffsetToReveal(object, 0.0).offset}');
    if (position.pixels > viewport.getOffsetToReveal(object, 0.0).offset) {
      // Move down to the top of the viewport
      Logger.debug('Move down to the top of the viewport');
      alignment = 0.0;
    } else if (position.pixels <
        viewport.getOffsetToReveal(object, 1.0).offset) {
      // Move up to the bottom of the viewport
      Logger.debug('Move up to the bottom of the viewport');
      alignment = 1.0;
    } else {
      // No scrolling is necessary to reveal the child
      Logger.debug('No scrolling is necessary to reveal the child');
      return;
    }
    Logger.debug('ensureVisible');
